...
public class OIDC {
  ...
  /**
   * Obtains an OpenID Connect Identity Token from the OIDC provider specified in <code>oidcAuthURL
   * </code>
   *
  ...
  public IDTokenResult getIDToken() throws IOException {
    ...
    if (!oidcDeviceCodeFlow) {
      final AuthorizationCodeFlow.Builder flowBuilder =
          new AuthorizationCodeFlow.Builder(
              ...
              .setCredentialCreatedListener(
                  (credential, tokenResponse) ->
                      memStoreFactory
                          .getDataStore("user")
                          .set(idTokenKey, tokenResponse.get(idTokenKey).toString()));

      final AuthorizationCodeInstalledApp app =
